...
export const SoftAurora: React.FC<SoftAuroraProps> = ({
  ...
  bandHeight = Defaults.bandHeight,
  bandSpread = Defaults.bandSpread,
  octaveDecay = Defaults.octaveDecay,
  layerOffset = Defaults.layerOffset,
  colorSpeed = Defaults.colorSpeed,
  ...
  React.useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      if (enableMouseInteraction) {
        mouse.x += MouseEasing * (target.x - mouse.x);
        mouse.y += MouseEasing * (target.y - mouse.y);
      }
  ...
  return <div ref={containerRef} className={classes.canvas} />;
};
